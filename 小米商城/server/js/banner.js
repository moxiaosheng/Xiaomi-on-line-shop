...
//       clearTimeout(timer);
  ...
  //显示banner图二级菜单 图片
    ...
    $(".banner_gwc1").on("mousemove",function(){
        $('.banner_gwc2').stop().slideDown(150).css({ 'box-shadow':'0px 6px 5px #292929'});
        $(this).css({'color':'#ff6700',"background":"#fff"});
        //$(this).css({"background":"#fff"})
    })
    $(".banner_gwc1").on("mouseout",function(){
        $('.banner_gwc2').stop().slideUp(150);
        $(this).css({"background":" #424242",'color':'#b6aebb'});

    })
    $(".banner_gwc2").on("mousemove",function(){
        $(this).stop().show();
        $('.banner_gwc1').css({"background":" #fff",'color':'#ff6700'});
    })
    $(".banner_gwc2").on("mouseout",function(){
        $(this).stop().slideUp(150);
        //$('.gwc1').css('color','#fff');
        $('.banner_gwc1').css({"background":" #424242",'color':'#b6aebb'});
    })
